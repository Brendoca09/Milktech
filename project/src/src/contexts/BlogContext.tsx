...
interface BlogContextType {
  posts: BlogPost[];
  addPost: (post: Omit<BlogPost, 'id' | 'publishedAt' | 'updatedAt' | 'slug' | 'views' | 'comments'>) => void;
...
export const BlogProvider: React.FC<BlogProviderProps> = ({ children }) => {
  ...
  const addPost = (postData: Omit<BlogPost, 'id' | 'publishedAt' | 'updatedAt' | 'slug' | 'views' | 'comments'>) => {
  ...
  const getBlogStats = (): BlogStats => {
    ...
    const totalComments = comments.filter(c => c.approved).length;

    const viewsByMonth = [
      { month: 'Jan', views: 1200 },
      { month: 'Fev', views: 1800 },
      { month: 'Mar', views: 2400 },
      { month: 'Abr', views: 3200 },
      { month: 'Mai', views: 2800 },
      { month: 'Jun', views: 3600 }
    ];
    ...
    const topPosts = posts
      ...
      .slice(0, 5)
      .map(post => ({ title: post.title.substring(0, 30) + '...', views: post.views }));
